#!/usr/bin/env python3
"""Extract Bahçeşehir, KSTU, and Istanbul Health programs from May 17 agentportal screenshots."""
...
OUT = Path(__file__).resolve().parent / "agentportal_programs_may17.json"
SUMMARY = Path(__file__).resolve().parent / "agentportal_programs_may17_summary.json"

# New batch only (exclude earlier Üsküdar screenshots same day).
GLOB = "Screenshot_2026-05-17_at_15.2*.png"
...
def norm_uni(token: str) -> str | None:
    ...
    if re.search(r"bah[cç]e[sş]ehir|bahcesehir|bauehir", t, flags=re.IGNORECASE):
        return "Bahçeşehir University"
    if re.search(r"\bkstu\b", t, flags=re.IGNORECASE):
        return "KSTU University"
    if re.search(
        r"istanbul\s*health|health\s*and\s*technology|iht\s*university",
        t,
        flags=re.IGNORECASE,
    ):
        return "Istanbul Health and Technology University"
    return None
...
def main() -> None:
    ...
    for p in paths:
        ...
        if not batch:
            # Retry: infer uni from filename order / second pass on header zone
    ...
    summary = {
        ...
        "expected_from_portal": {
            "Bahçeşehir University": 118,
            "KSTU University": 50,
            "Istanbul Health and Technology University": 26,
        },
