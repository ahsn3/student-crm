...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Elderly Care Services'))
      AND COALESCE(p.degree, '') = 'Diploma'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
INSERT INTO program (id, university_id, name, degree, language, cost, years, created_at, available, name_ar, before_discount, cash_price, updated_at)
SELECT gen_random_uuid(), u.id,
  'Health Information System Technics',
  'Diploma',
  'Turkish',
  2800,
  2,
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Health Information System Technics'))
      AND COALESCE(p.degree, '') = 'Diploma'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Medical Promotion And Marketing'))
      AND COALESCE(p.degree, '') = 'Diploma'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
INSERT INTO program (id, university_id, name, degree, language, cost, years, created_at, available, name_ar, before_discount, cash_price, updated_at)
SELECT gen_random_uuid(), u.id,
  'Addiction Consultancy And Rehabilitation With Thesis',
  'Master',
  'Turkish',
  4800,
  2,
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Addiction Consultancy And Rehabilitation With Thesis'))
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Applied Psychology'))
      AND COALESCE(p.degree, '') = 'Master'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Criminal Justice With Thesis'))
      AND COALESCE(p.degree, '') = 'Master'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Electrical And Electronic Engineering With Thesis'))
      AND COALESCE(p.degree, '') = 'Master'
      AND COALESCE(p.language, '') = 'English'
  );
...
INSERT INTO program (id, university_id, name, degree, language, cost, years, created_at, available, name_ar, before_discount, cash_price, updated_at)
SELECT gen_random_uuid(), u.id,
  'The Thought, History, And Literature Of Islamic Civilization',
  'PhD',
  'Turkish',
  13300,
  4,
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('The Thought, History, And Literature Of Islamic Civilization'))
      AND COALESCE(p.degree, '') = 'PhD'
      AND COALESCE(p.language, '') = 'Turkish'
  );
...
WHERE lower(trim(u.name)) = lower(trim('Üsküdar University'))
  AND NOT EXISTS (
    SELECT 1 FROM program p
    WHERE p.university_id = u.id
      AND lower(trim(p.name)) = lower(trim('Biosecurity With Thesis'))
      AND COALESCE(p.degree, '') = 'Master'
      AND COALESCE(p.language, '') = 'Turkish'
  );
